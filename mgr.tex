...
\autor{Aleksander Wiśniewski}{ 451468}

\title{Sequences defined by weighted logics}
\titlepl{Ciągi definiowane przez logiki ważone}
\kierunek{Computer Science}
\opiekun{dr Filip Mazowiecki}
...
% Słowa kluczowe:
\keywords{automata theory, weighted logic, monadic second order logic, sequences}
...
\label{ExFibCont}
    ...
    \begin{figure}[ht]
        \centering
        \begin{tikzpicture}
            \node[state, initial, accepting] (q10) {$q_{1_0}$};
            \node[state, accepting, above of=q10] (q11) {$q_{1_1}$};
            \node[state, accepting, right of=q10] (q20) {$q_{2_0}$};
            \node[state, accepting, right of=q11] (q21) {$q_{2_1}$};
            \node[state, right of=q20] (q30) {$q_{3_0}$};
            \node[state, right of=q21] (q31) {$q_{3_1}$};
            \draw 
            (q10) edge[loop above] node{$a$} (q10)
            (q10) edge[above] node{$a$} (q21)
            (q11) edge[bend right, below] node{$a$} (q10)
            (q11) edge[above] node{$a$} (q21)
            (q20) edge[above] node{$a$} (q10)
            (q21) edge[above] node{$a$} (q10)
            (q20) edge[above] node{$a$} (q31)
            (q21) edge[above] node{$a$} (q31);
            \end{tikzpicture}
        \caption{Nondeterministic ``counting'' automaton that skips variable $X$ from $\varphi(X)$}
        \label{fig:my_label2}
    \end{figure}
